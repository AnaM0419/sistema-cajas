...
c1, c2 = st.columns(2)
c1.metric("Efectivo", db.dinero(saldo["efectivo"]))
c2.metric("Sistema", db.dinero(saldo["sistema"]))

OPERACIONES = {
    "Retiro — el cliente se lleva efectivo": "retiro",
    "Depósito — el cliente entrega efectivo": "deposito",
    "Sacar plata de la cuenta": "salida_sistema",
    "Sacar dinero de la caja": "salida_efectivo",
    "Acreditar dinero a la cuenta": "acreditacion",
}

etiqueta = st.radio("¿Qué pasó?", list(OPERACIONES), index=0)
tipo = OPERACIONES[etiqueta]
...
with st.form("movimiento", clear_on_submit=True):
    ...
    if tipo == "retiro":
        ...
        st.caption("El dinero sale del efectivo de la caja y sube el saldo del sistema. "
    ...
    guardar = st.form_submit_button("Registrar", type="primary", use_container_width=True)
...
if guardar:
    ...
    if tipo in ("retiro", "salida_efectivo", "acreditacion") and monto > float(saldo["efectivo"]):
    ...
    if errores:
        ...
        try:
            ...
            st.success(
                ...
                f"Efectivo {mov['delta_efectivo']:+.2f} · Sistema {mov['delta_sistema']:+.2f}"
            )
            st.rerun()
...
with st.expander("Anular un movimiento"):
    st.caption("No se borra nada: se registra el movimiento espejo y los dos quedan "
               "en el historial.")
